...
On success, the node may send an acknowledgment (shape TBD) per command type—e.g. echo the applied `action` or a simple `ok` flag so the testbench can confirm the update.
